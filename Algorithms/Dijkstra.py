...
def dijkstra(adjacency_matrix, start_vertex: int, end_vertex: int, full_info: bool = False):
    ...
    while len(distances_temp):
        ...
        for neighbor_i, neighbor_vertex in enumerate(adjacency_matrix[curr_vertex_ind]):
            # Первое условие нужно для того, чтобы не было ошибки KeyError для уже просмотренных (удаленных) вершин
            if neighbor_i in distances_temp:
                distance_to_neighbor = distances_temp[curr_vertex_ind].distance + neighbor_vertex
...
if __name__ == '__main__':
    ...
    for i in range(1, iterations_count+1):
        ...
        for i in range(len(matrix)):
            assert dijkstra(matrix, start_vertex=0, end_vertex=i, full_info=True) == \
                   test(matrix, start_vertex=0, end_vertex=i)

...
class KthOrderStatistics:
    ...
    def k_order_statistics(cls, lst: list, k: int, randomized: bool = True):
        ...
        Using with randomized=False will choose deterministic select (median of medians) algorithm.
        Sometimes it called BFPRT because of the last names of the authors.
        Complexity of both algorithms: O(n).
        Memory-complexity of random select is O(n), but memory-complexity of deterministic select is O(n*log_n).
        """
        cls.__lst = lst.copy()
        cls.__k = k - 1
    ...
    def __determined_k_order_statistics(cls, left, right, lst=None, k=None):
        ...
        if k < m:
            ...
            return cls.__determined_k_order_statistics(0, right-m-1, k=k-m-1, lst=lst[m+1:right+1])
    ...
    def __pick_pivot(cls, lst):
        ...
        median_lst = lst[2:len(lst):5] if len(lst) >= 5 else [lst[randint(0, len(lst)-1)]]
...
if __name__ == '__main__':
    ...
    for i in range(1, iterations_count+1):
        ...
        assert sorted(lst)[k-1] == KthOrderStatistics.k_order_statistics(lst, k, randomized=True)
        assert sorted(lst)[k-1] == KthOrderStatistics.k_order_statistics(lst, k, randomized=False)

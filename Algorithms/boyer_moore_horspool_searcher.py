...
def boyer_moore_horspool_searcher(subseq, seq) -> int:
    ...
    Average-complexity: E(T(m, n)) = O(m/Z), where Z = len(set(subseq + seq)).
    Best case: O(n/m), worse case: O(m*n).
...
if __name__ == "__main__":
    print(
        "Description of function boyer_moore_horspool_searcher:\n",
    ...
    for i in range(1, iterations_count + 1):
        ...
        sequence = [
            randint(-2 * sequence_length, 2 * sequence_length) for _ in range(sequence_length)
        ]
        ...
        if is_slice_from_sequence:
            ...
            subsequence = [
                randint(-sequence_length, sequence_length) for _ in range(subsequence_length)
            ]
        sequence = " ".join(map(str, sequence))
        subsequence = " ".join(map(str, subsequence))

        assert sequence.find(subsequence) == boyer_moore_horspool_searcher(
